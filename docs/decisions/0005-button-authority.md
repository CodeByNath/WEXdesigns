...
Hover is a restrained change; pressed is the third, stronger state. Pressed and Focus consume the same reusable two-pixel WEX state boundary and two-pixel internal gap. That boundary replaces the normal visible Button edge at the fixed outer perimeter; inset layers may complete its thickness and separation but must not leave the normal border visible outside a smaller inner state rectangle. The treatment never increases visible Button width or height; Pressed remains transient native `:active` while Focus remains the independent accessibility overlay. Disabled controls use native disabled behaviour and the accepted universal `--wex-opacity-disabled` once at the outer control; they expose no hover, pressed, or focus state.
