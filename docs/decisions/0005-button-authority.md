...
Hover is a restrained change; pressed is the third, stronger state. Pressed and Focus consume the same reusable two-pixel WEX ring treatment and two-pixel internal gap. The treatment is an inset layer inside the existing Button boundary, so it never increases visible Button width or height; Pressed remains transient native `:active` while Focus remains the independent accessibility overlay. Disabled controls use native disabled behaviour and the accepted universal `--wex-opacity-disabled` once at the outer control; they expose no hover, pressed, or focus state.
